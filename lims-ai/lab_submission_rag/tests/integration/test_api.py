...
class TestAPIEndpoints:
    ...
    def mock_rag_system(self) -> None:
        ...
        mock_rag.get_system_status = AsyncMock(
            return_value={
                "status": "healthy",
                "total_documents": 5,
                "total_chunks": 25,
    ...
    def test_process_document_success(self, mock_rag, client, mock_rag_system, sample_pdf_file) -> None:
        ...
        response = client.post(
            "/process-document", files={"file": (filename, file_content, content_type)}
        )
    ...
    def test_query_submission_minimal_params(self, mock_rag, client, mock_rag_system) -> None:
        ...
        query_data = {"query": "What are the requirements for lab submission?"}
    ...
    def test_legacy_process_endpoint(self, mock_rag, client, mock_rag_system, sample_pdf_file) -> None:
        ...
        filename, file_content, content_type = sample_pdf_file
        response = client.post("/process", files={"file": (filename, file_content, content_type)})
    ...
    def test_query_with_submission_filter(self, mock_rag, client, mock_rag_system) -> None:
        ...
        query_data = {"query": "What tests were requested?", "submission_id": "SUB-12345"}
