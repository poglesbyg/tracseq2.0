...
class LabSubmissionRAG:
    ...
    async def process_document(self, file_path: str | Path) -> ExtractionResult:
        ...
        try:
            ...
            logger.info(
                f"Document processing completed. Success: {extraction_result.success}, "
    ...
    async def query_submissions(
        ...
        try:
            ...
            await self._log_query(
                query, answer, session_id, time.time() - start_time, len(relevant_chunks)
            )
            ...
            error_message = "I apologize, but I encountered an error while processing your query. Please try rephrasing your question or contact support if the issue persists."
            ...
            async with db_manager.get_session() as session:
                ...
                if any(
                    ...
                    count = await repo.get_sample_count(
                        sample_type=sample_type, storage_condition=storage_condition
                    )
                ...
                # Sample statistics queries
                    ...
                    phrase in query_lower
                    for phrase in ["sample statistics", "sample breakdown", "sample summary"]
                ):
                ...
                # Submission queries
                    ...
                    phrase in query_lower
                    for phrase in ["how many submissions", "submission count", "total submissions"]
                ):
                ...
                # Search queries
                    ...
                    phrase in query_lower for phrase in ["search for", "find samples", "look for"]
                ):
                    # Extract search term (simple implementation)
                    search_terms = (
                        query_lower.replace("search for", "")
                        .replace("find samples", "")
                        .replace("look for", "")
                        .strip()
                    )
                    if search_terms:
                        ...
                        if samples:
                            response = (
                                f"Found **{len(samples)}** samples matching '{search_terms}':\n\n"
                            )
    ...
    async def export_submission_data(self, submission: LabSubmission, format: str = "json") -> str:
        """
