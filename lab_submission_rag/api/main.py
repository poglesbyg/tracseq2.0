...
async def query_submission_information(request: QueryRequest):
    ...
    try:
        answer = await rag_system.query_submissions(
            query=request.query,
            filter_metadata=(
                {"submission_id": request.submission_id} if request.submission_id else None
            ),
...
async def get_sample_count(
    ...
    try:
        ...
        async with db_manager.get_session() as session:
            ...
            return {
                "count": count,
                "filters": {"sample_type": sample_type, "storage_condition": storage_condition},
            }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
                ...
                results.append(
                    {
                        ...
                        "sample_type": (
                            submission.sample_type.value if submission.sample_type else None
                        ),
