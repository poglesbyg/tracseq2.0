...
class Settings(BaseSettings):
    ...
    llm_provider: str = Field(
        default="openai", description="LLM provider to use (openai, anthropic, ollama)"
    )
    model_name: str = Field(default="gpt-3.5-turbo", description="Name of the LLM model to use")

    # Ollama Settings
    use_ollama: bool = Field(
        default=False, description="Whether to use Ollama for local LLM inference"
    )
    ollama_model: str = Field(default="llama2:7b", description="Ollama model to use")
    ...
    embedding_model: str = Field(
        default="sentence-transformers/all-MiniLM-L6-v2", description="Model to use for embeddings"
    )

    # Document Processing
    chunk_size: int = Field(default=1000, description="Size of document chunks in characters")
    chunk_overlap: int = Field(default=200, description="Overlap between chunks in characters")

    # Vector Store
    vector_store_path: Path = Field(
        default=Path("data/vector_store"), description="Path to vector store files"
    )

    # File Storage
    upload_dir: Path = Field(default=Path("uploads"), description="Directory for uploaded files")
    export_dir: Path = Field(default=Path("exports"), description="Directory for exported files")
    ...
    max_search_results: int = Field(
        default=5, description="Maximum number of search results to retrieve"
    )
    llm_temperature: float = Field(
        default=0.3, description="Temperature for LLM responses (0.0-1.0)"
    )
    max_tokens: int = Field(default=2048, description="Maximum tokens for LLM responses")
    similarity_threshold: float = Field(
        default=0.5, description="Minimum similarity score for chunk inclusion"
    )
    batch_size: int = Field(default=5, description="Batch size for processing multiple documents")
    export_directory: str = Field(
        default="exports", description="Directory for exporting processed data"
    )
    ...
    database_host: str = Field(default="host.docker.internal", description="Database host")
    database_port: int = Field(default=5432, description="Database port")
    database_name: str = Field(default="lab_manager", description="Database name")
    ...
    table_prefix: str = Field(
        default="rag_", description="Prefix for RAG system tables to avoid conflicts"
    )
