#!/usr/bin/env python3
"""
Simple Frontend API Bridge for RAG Submissions - Fixed Version
...
# Keep the old JSON endpoint for testing purposes
@app.post("/api/rag/process-text", response_model=DocumentProcessResponse)
async def process_document_text(request: DocumentProcessRequest):
    """Process document text and extract laboratory information using regex-based extraction"""
    start_time = time.time()
    
    try:
        # Extract information using basic regex patterns
        extraction_result = extract_sample_info_basic(request.text)
