...
class ABTestManager:
    ...
    async def calculate_results(self, test_id: str) -> Dict[str, ABTestResult]:
        ...
        with self.SessionLocal() as session:
            interactions = session.query(ABTestInteractionRecord).filter(
                ABTestInteractionRecord.test_id == test_id
            ).all()
    ...
    async def stop_test(self, test_id: str, reason: str = "Manual stop") -> bool:
        ...
        # Update database
        with self.SessionLocal() as session:
            record = session.query(ABTestRecord).filter(
                ABTestRecord.test_id == test_id
            ).first()
    ...
    def _calculate_variant_results(
        self,
        variant_id: str,
        interactions: List[ABTestInteractionRecord],
        config: ABTestConfig
    ) -> ABTestResult:
    ...
    def _calculate_statistical_significance(
        ...
        treatment_value = getattr(treatment, metric, None) or treatment.custom_metrics.get(metric, 0)
    ...
    async def _store_test_results(self, test_id: str, results: Dict[str, ABTestResult]):
        ...
        results_data = {
            variant_id: asdict(result) for variant_id, result in results.items()
        }
    ...
    async def _pause_test(self, test_id: str, reason: str):
        ...
        logger.error(
            "A/B test paused due to guardrail violation",
            test_id=test_id,
            reason=reason
        )
    ...
    def _load_active_tests(self):
        ...
        with self.SessionLocal() as session:
            active_records = session.query(ABTestRecord).filter(
                ABTestRecord.status == TestStatus.RUNNING.value
            ).all()
