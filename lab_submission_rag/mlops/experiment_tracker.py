...
class ExperimentTracker:
    ...
    async def get_experiment_metrics(self, experiment_id: str) -> pd.DataFrame:
        ...
        with self.SessionLocal() as session:
            records = session.query(MetricsRecord).filter(
                MetricsRecord.experiment_id == experiment_id
            ).order_by(MetricsRecord.step).all()
    ...
    async def plot_experiment_metrics(
        ...
        available_metrics = [m for m in metrics if m in metrics_df.columns and metrics_df[m].notna().any()]
