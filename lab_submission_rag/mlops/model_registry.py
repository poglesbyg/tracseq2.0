...
class ModelRegistry:
    ...
    async def register_model(
        ...
        try:
            ...
            logger.error("Failed to register model", model_id=metadata.model_id, error=str(e))
            raise
    ...
    async def update_model_status(self, model_id: str, version: str, status: ModelStatus) -> bool:
        ...
        with self.SessionLocal() as session:
            record = (
                ...
                .filter(ModelRecord.model_id == model_id, ModelRecord.version == version)
                .first()
            )
            ...
            logger.info(
                "Model status updated", model_id=model_id, version=version, status=status.value
            )
            return True
    ...
    async def promote_model(self, model_id: str, version: str, to_status: ModelStatus) -> bool:
        ...
        promotion_rules = {
            ...
            ModelStatus.PRODUCTION: lambda m: m.status == ModelStatus.STAGING
            and m.error_rate < 0.05,
        }
    ...
    async def delete_model(self, model_id: str, version: str) -> bool:
        ...
        if metadata.status == ModelStatus.PRODUCTION:
            logger.warning("Cannot delete production model", model_id=model_id, version=version)
            return False
