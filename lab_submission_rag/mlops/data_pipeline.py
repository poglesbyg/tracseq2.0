...
class DataPipeline:
    ...
    async def get_pipeline_run(self, run_id: str) -> Optional[PipelineRun]:
        ...
        with self.SessionLocal() as session:
            record = session.query(PipelineRunRecord).filter(
        ...
        for column_name, column_def in schema.columns.items():
            ...
            if df[column_name].dtype in ['int64', 'float64']:
                ...
                report.column_stats[column_name] = {
                    ...
                    "missing_count": df[column_name].isnull().sum(),
                    "unique_values": df[column_name].nunique(),
                    "most_frequent": df[column_name].mode().iloc[0] if not df[column_name].empty else None
                }
    ...
    async def get_schema(self, schema_id: str) -> Optional[DataSchema]:
        ...
        with self.SessionLocal() as session:
            record = session.query(DataSchemaRecord).filter(
                DataSchemaRecord.schema_id == schema_id
            ).first()
    ...
    async def get_feature_config(self, config_id: str) -> Optional[FeatureEngineeringConfig]:
        ...
        with self.SessionLocal() as session:
            record = session.query(FeatureEngineeringConfigRecord).filter(
                FeatureEngineeringConfigRecord.config_id == config_id
            ).first()
    ...
    async def _execute_pipeline(self, pipeline_run: PipelineRun):
        ...
        try:
            ...
            quality_report = await self.assess_data_quality(
                pipeline_run.input_data_path,
                pipeline_run.schema_id
            )
    ...
    async def _preprocess_data(self, df: pd.DataFrame, quality_report: DataQualityReport) -> pd.DataFrame:
    ...
    async def _engineer_features(self, df: pd.DataFrame, config: FeatureEngineeringConfig) -> pd.DataFrame:
        ...
        if config.categorical_features:
            categorical_columns = [col for col in config.categorical_features if col in features_df.columns]
        ...
        for transformation in config.custom_transformations:
            ...
            if transform_name in self.transformation_functions:
                features_df = self.transformation_functions[transform_name](features_df, transformation)
        
        return features_df
    
    async def _select_features(self, df: pd.DataFrame, config: FeatureEngineeringConfig) -> pd.DataFrame:
    ...
    async def _update_pipeline_run(self, run: PipelineRun):
        ...
        with self.SessionLocal() as session:
            record = session.query(PipelineRunRecord).filter(
                PipelineRunRecord.run_id == run.run_id
            ).first()
