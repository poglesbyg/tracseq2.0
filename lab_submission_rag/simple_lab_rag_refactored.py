...
try:
    ...
    from simple.llm_interface import DemoLLMInterface, SimpleLLMInterface
...
class SimpleVectorStore:
    ...
    def add_document(self, submission_id: str, text: str, metadata: Dict[str, Any]):
        ...
        try:
            ...
            # Add to collection
            self.collection.add(
                embeddings=[embedding], documents=[text], metadatas=[metadata], ids=[submission_id]
            )
    ...
    def __init__(self, use_ollama=True, use_openai=True, data_dir="./data"):
        ...
        try:
            self.llm = SimpleLLMInterface(
                model=os.getenv("OLLAMA_MODEL", "llama3.2:3b"), use_openai_fallback=use_openai
            )
        except RuntimeError:
    ...
    def process_document(self, file_path: Union[str, Path]) -> ExtractionResult:
        ...
        try:
            ...
            if not file_path.exists():
                return ExtractionResult(success=False, error=f"File not found: {file_path}")
        ...
        try:
            ...
            for result in search_results:
                context_parts.append(
                    f"Document: {result['metadata'].get('source_document', 'Unknown')}"
                )
    ...
    def export_submissions(self, format: str = "json") -> str:
        ...
        try:
            if format.lower() == "json":
                export_file = (
                    self.data_dir
                    / f"submissions_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                )
...
def main():
    ...
    if result.success:
        ...
        print("   Query: 'Who is the submitter?'")
