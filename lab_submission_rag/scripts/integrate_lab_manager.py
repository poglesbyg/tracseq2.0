    ...
    try:
        async with db_manager.get_session() as session:
            ...
            for table_name, columns in schema_info.items():
                ...
                for col in columns:
                    logger.info(
                        f"  - {col['column']}: {col['type']} ({'NULL' if col['nullable'] == 'YES' else 'NOT NULL'})"
                    )
...
async def suggest_integration_strategy():
    ...
    submission_tables = [
        ...
        if any(word in t.lower() for word in ["submission", "request", "order"])
    ]

    if sample_tables:
        logger.info(f"\nFound existing sample-related tables: {sample_tables}")
        logger.info(
            "RECOMMENDATION: Consider creating views or adapters to connect RAG data with existing sample data"
        )

    if submission_tables:
        logger.info(f"\nFound existing submission-related tables: {submission_tables}")
        logger.info(
            "RECOMMENDATION: Consider linking RAG submissions to existing submission workflow"
        )
