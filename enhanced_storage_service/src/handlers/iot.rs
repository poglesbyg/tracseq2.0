...
pub async fn get_sensor_data(
    ...
    let start_time = Utc::now() - Duration::hours(hours_back as i64);
    ...
    let stats = if !readings.is_empty() {
        calculate_sensor_statistics(&readings)
    } else {
        SensorStatistics {
            min_value: 0.0,
            max_value: 0.0,
...
pub async fn record_sensor_reading(
    ...
    let reading = sqlx::query_as::<_, SensorReading>(
        ...
        INSERT INTO sensor_readings (
            id, sensor_id, value, unit, timestamp, metadata
        )
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING *
        "#,
    ...
    .bind(&sensor_id)
    .bind(request.value)
    .bind(request.unit.unwrap_or("unknown".to_string()))
    ...
    sqlx::query(
        "UPDATE iot_sensors SET last_reading = $1, updated_at = NOW() WHERE sensor_id = $2"
    )
    ...
    // Check for alerts
    if let Err(e) = check_sensor_alerts(&state, &sensor, &reading).await {
        warn!("Failed to check sensor alerts: {}", e);
    }
...
pub async fn get_alerts(
    ...
    let mut query_builder = sqlx::QueryBuilder::new(
        "SELECT id, sensor_id, alert_type, severity, message, threshold_value, 
         actual_value, resolved, created_at, resolved_at 
         FROM iot_alerts WHERE 1=1"
    );
    ...
    if let Some(severity) = &query.severity {
        ...
        query_builder.push_bind(severity);
    }
    ...
    let alerts = query_builder
        .build_query_as::<IoTAlert>()
        .fetch_all(&state.storage_service.db.pool)
        .await?;

    // Get total count
    let total_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM iot_alerts")
        .fetch_one(&state.storage_service.db.pool)
        .await?;
