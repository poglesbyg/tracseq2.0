...
type ConfigStore = Arc<RwLock<HashMap<String, ConfigEntry>>>;
type TemplateStore = Arc<RwLock<HashMap<String, ConfigTemplate>>>;
...
// Bulk update configurations for a service
async fn bulk_update_service_config(
    Path((service_name, environment)): Path<(String, String)>,
    State(state): State<AppState>,
...
pub fn create_app() -> Router {
    ...
    Router::new()
        ...
        .route("/configs/:service_name/:key", put(update_config).delete(delete_config))
        .route("/configs/:service_name/:environment", get(get_service_config))
        .route("/configs/:service_name/:environment/bulk", put(bulk_update_service_config))
        .layer(
            ServiceBuilder::new()
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    state.initialize_default_configs().await;
    ...
    info!("🚀 TracSeq Configuration Service starting on {}", addr);
