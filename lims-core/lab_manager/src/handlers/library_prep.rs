...
impl LibraryPrepManager {
    ...
    pub async fn complete_library_prep(&self, prep_id: Uuid, metrics: LibraryPrepMetrics) -> Result<LibraryPreparation, sqlx::Error> {
        ...
        .bind(prep_id)
        .bind(metrics.final_concentration_ng_ul)
        .bind(metrics.final_volume_ul)
        .bind(metrics.total_yield_ng)
        .bind(serde_json::to_value(&metrics.quality_metrics).ok())
        .fetch_one(&self.pool)
        .await
    }
